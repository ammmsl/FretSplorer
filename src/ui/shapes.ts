// Movable-shape helpers (/ui) — PURE shape -> Grip projection at a chosen anchor fret,
...
// it at anchor fret N realises a concrete Grip: a fretted string lands at N + offset, an
...
import type { Grip, StringGrip } from './grip';

/**
 * Realise a movable shape as a concrete Grip at the given anchor fret. `fret` entries are
 * placed at `anchorFret + offset` (clamped to open at fret 0); `open` -> open drone; `mute`
 * -> muted. The result is index-aligned to the tuning (one StringGrip per shape-string).
 */
export function shapeToGrip(shape: MovableShape, anchorFret: number): Grip {
