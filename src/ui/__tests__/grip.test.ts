// Grip helpers — the per-string interaction state transitions + PlacedPosition[]
// derivation (docs/08 decision e; docs/09 UI#4). Validated against known grips.
...
describe('emptyGrip / isGripEmpty', () => {
  it('builds one unplayed entry per string and reads as empty', () => {
    const g = emptyGrip(6);
    expect(g).toHaveLength(6);
    expect(g.every((s) => s.kind === 'unplayed')).toBe(true);
    expect(isGripEmpty(g)).toBe(true);
  });

  it('a grip of only muted strings is still empty (sounds nothing)', () => {
    let g = emptyGrip(3);
  ...
  it('a grip with one open string is NOT empty', () => {
    const g = cycleNutMarker(emptyGrip(3), 0); // -> open
    expect(isGripEmpty(g)).toBe(false);
  });
});

describe('placeFret / removeNote (one note per string)', () => {
  it('places a fretted note and replaces an existing one on that string', () => {
    let g = placeFret(emptyGrip(6), 2, 3);
  ...
  it('normalises fret 0 to the open marker', () => {
    const g = placeFret(emptyGrip(6), 0, 0);
    expect(g[0]).toEqual({ kind: 'open' });
  });

  it('removes a placed note back to unplayed', () => {
    const g = removeNote(placeFret(emptyGrip(6), 1, 4), 1);
...
describe('cycleNutMarker', () => {
  ...
  it('a fretted string cycles to open (a clean way back)', () => {
    const g = cycleNutMarker(placeFret(emptyGrip(2), 0, 5), 0);
    expect(g[0].kind).toBe('open');
  });
});

describe('gripToPlaced (the identify() input)', () => {
  it('Open-G all-open -> six PlacedPositions at fret 0', () => {
    const allOpen: Grip = openG.openStrings.map(() => ({ kind: 'open' as const }));
    const placed = gripToPlaced(allOpen);
...
describe('soundingPitch', () => {
  it('returns open + fret for fretted, open pitch for open, null otherwise', () => {
    ...
    g = cycleNutMarker(emptyGrip(6), 5); // open
    expect(soundingPitch(g, openG, 5)).toBe(38);
    expect(soundingPitch(emptyGrip(6), openG, 5)).toBeNull();
  });
});
